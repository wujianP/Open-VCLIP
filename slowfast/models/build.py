...
class BasicClipVideo(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        self.model, _, self.preprocess = open_clip.create_model_and_transforms('ViT-B-32',
                                                                               pretrained=cfg.TEST.CUSTOM_LOAD_FILE)
    ...
    def text_prompt(self, data_file):
        ...
        actionclip_text_aug = [f"a photo of action {{}}", f"a picture of action {{}}", f"Human action of {{}}",
                               f"{{}}, an action",
                               f"{{}} this is an action", f"{{}}, a video of action", f"Playing action of {{}}",
                               f"{{}}",
                               f"Playing a kind of action, {{}}", f"Doing a kind of action, {{}}",
                               f"Look, the human is {{}}",
                               f"Can you recognize the action of {{}}?", f"Video classification of {{}}",
                               f"A video of {{}}",
...
def build_model(cfg, gpu_id=None):
    ...
    # Construct the model
    from IPython import embed
    embed(header='build model')
    model = BasicClipVideo(cfg)
